...
pub async fn serve(config: ServeConfig) -> Result<BoundEndpoints, IpcError> {
    ...
    let jsonrpc_handle = spawn_jsonrpc_unix(&jsonrpc_path, Arc::clone(&state), shutdown_rx.clone());

    let tarpc_socket = if config.enable_tarpc {
        #[cfg(feature = "tarpc")]
        {
...
mod tests {
    ...
    async fn unix_jsonrpc_health_liveness() {
        ...
        spawn_jsonrpc_unix(&socket_path, state, shutdown_rx);
